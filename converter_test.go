...
func TestDefaultConverter(t *testing.T) {
	...
	for key, value := range event.Contexts["extra"] {
...
func TestAttrToSentryEvent(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"context_group": {
			attr: slog.Attr{Key: "extra", Value: slog.GroupValue(
				slog.Attr{Key: "key1", Value: slog.StringValue("value1")},
				slog.Attr{Key: "key2", Value: slog.StringValue("value2")},
			)},
			expected: &sentry.Event{Contexts: map[string]map[string]any{
				"extra": map[string]any{
					"key1": "value1",
					"key2": "value2",
				}},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if len(tc.expected.Contexts) == 0 {
				assert.Empty(t, event.Contexts)
			} else {
				assert.Equal(t, tc.expected.Contexts, event.Contexts)
			}
		})
	}
}
